...
import trabalhos from './data.json';
import CardWork from '../../components/CardWork';

import './style.css';

const Works = ()=>{
    const { data } = trabalhos;
    ...
    useEffect(()=>{
        let listFilters = data.map(res=> res.filter)
                    ...
                    <div className="float-right text-white overflow-hidden">
                        {
                            filters.map(res=>(
                                <div className="d-inline mr-3" key={res}>
                                    ...
                                    />
                                    <label htmlFor={`radio-${res}`} 
                                    className={`text-uppercase label-filter mr-2 small py-2 ${res === 'todos' ? 'active' : ''}`}>{res}</label>
                                </div>
                            ))
                        }
                    </div>
